...
df = pd.read_csv("C:\\Users\\zhero\\Desktop\\CUNY\\DATA608\\final\\datasets\\daily_data.csv")
df = df[df.close_x != 0]
...
def get_market_cap(t):
    ...
    no_shares_b = df[(df.ticker==t) & (df.date==end_date)]['shares_outstanding_b_x'].values[0]
    price_b =df[(df.ticker==t) & (df.date==end_date)]['close_rmb'].values[0]
    b_cap = no_shares_b * price_b
    no_shares_a = df[(df.ticker==t) & (df.date==end_date)]['shares_outstanding_a_x'].values[0]
    price_a =df[(df.ticker==t) & (df.date==end_date)]['close_y'].values[0]
...
def update_beta_chart(ticker):
    ...
    reg = LinearRegression().fit(np.vstack(df['return_monthly_ix1']), df['return_monthly_a'])
    df['bestfit_a'] = reg.predict(np.vstack(df['return_monthly_ix1']))
